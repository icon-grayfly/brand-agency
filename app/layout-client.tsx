...
export default function LayoutClient({ children }: { children: React.ReactNode }) {
  ...
  // Helper to highlight active navigation tabs cleanly
  const isActive = (path: string) => pathname === path ? 'text-cyan-400 font-bold' : 'text-slate-300 hover:text-cyan-400';

  return (
    <div className="min-h-screen flex flex-col bg-slate-950 selection:bg-cyan-500 selection:text-slate-900 overflow-x-hidden">
      ...
      <header className="fixed top-0 inset-x-0 z-50 backdrop-blur-xl bg-slate-950/80 border-b border-slate-900/60 h-20">
        <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium">
            ...
            <Link href="/work" className={`${isActive('/work')} transition-colors`}>Our Work</Link>
            <Link href="/events" className={`${isActive('/events')} transition-colors`}>Agency Events</Link>
      ...
      {/* GLOBAL SLIDING MARQUEE BRANDS TRACK (Consistent across all pages) */}
      <div className="w-full pt-20 bg-slate-900/30 border-b border-slate-900 overflow-hidden relative select-none z-10">
      ...
      {/* VIBRANT VIEWPORT CONTENT PORTAL */}
      <main className="flex-grow z-10 relative pt-8">
        {children}
      </main>

      {/* GLOBAL BRAND FOOTER WITH INTEGRATED MATCHING LOGO */}
      <footer className="border-t border-slate-900 bg-slate-950 py-16 px-6 z-20 relative mt-auto">
      ...
      {/* INTERACTIVE FLOATING WHATSAPP CHAT BADGE */}
      <a 
        href="https://wa.me/2348091000449?text=Hi%20Star%20Station,%20we%20want%20to%20engage%20your%20services."
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 right-6 z-50 bg-[#25D366] text-white w-14 h-14 rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300"
